...
# ╔═╡ 38a5d6d8-33e2-11ec-3161-655c73432458
begin
	using ExponentialUtilities:expv
	using DifferentialEquations:ODEProblem, solve
	using Plots
	using ForwardDiff:jacobian
	using LinearAlgebra:mul!, Diagonal, diagind
end

# ╔═╡ 2b808926-6c30-496a-a82d-9ddea432f1b8
md"""
## Explore jacobians and matrix exponentials to approximatively solve and ODE
"""

# ╔═╡ cdd185bd-bfb7-44cf-8111-3d1644e30e25
# ODE that we want to solve and approximate
#=
# Lorenz system
...
# end
function f!(dx,x,A,t)
	dx[1] = x[1] * (A[1,1] + transpose(A[:,2]) * x)
	dx[2] = x[2] * (A[2,1] + transpose(A[:,3]) * x)
	dx[3] = x[3] * (A[3,1] + transpose(A[:,4]) * x)
end


# Linear system
# f!(dx,x,A,t) = mul!(dx,A,x)

# ╔═╡ 7845fa6c-defc-410f-9466-1a465e018bb4
# solve ODE
begin
	tspan = (0.0, 10.0)
	x0 = [0.1, 1., 1.1]
	# μ_θ = [10.0, 28.0, 8/3]    # parameters for Lorenz attractor
	# μ_θ = -rand(length(x0), length(x0))   # parameters for linear system
	μ_θ = rand(length(x0), length(x0)+1)   # parameters for Lotka-Volterra
	μ_θ[:, 1] += ones(length(x0))*1
	A = μ_θ[:, 2:end]
	A[diagind(A)] = -sum(A, dims=2)
	μ_θ[:, 2:end] = A

	prob = ODEProblem(f!, x0, tspan, μ_θ)
	sol = solve(prob)
end

# ╔═╡ 4085af09-6011-47d7-b617-54437cc5ac18
A, μ_θ[:,1]

# ╔═╡ ff5ee4de-e830-4ef0-971c-b3d0b87d7231
# plot ODE solution
plot(sol)

# ╔═╡ cd5ca694-bb32-401f-8374-e210db04447c
# Compute Jacobian
begin
	fx! = (dx, x) -> f!(dx, x, μ_θ, 0)
	dx = similar(x0)
	J_x = x -> jacobian(fx!, dx, x)
end

# ╔═╡ 8182f72e-e2aa-4c1a-ba45-56523e3c16f5
# Integration of ODE based on linearized approximation
begin
	dt = 0.0001
	nt = round(Int, tspan[2]/dt)
	x_approx = zeros(nt, length(x0))
	x_approx[1, :] = x0
	for i = 2:nt
		x_approx[i, :] = expv(dt, J_x(x_approx[i-1, :]), x_approx[i-1, :])
	end
end

# ╔═╡ d6f2dc80-23b1-4e41-9ba1-b8b43ab288e7
plot((1:nt)*dt, x_approx)

# ╔═╡ 1c3717dd-5857-4e0b-82fa-e5e7a9f24008
begin 
	plot((1:nt)*dt, x_approx)
	plot!(sol)
end
