...
# ╔═╡ 2140e861-db8c-4a0d-b83a-4a427f7535cb
begin
	using FFTW
	using Plots
	using StatsBase
...
# ╔═╡ ffda8474-497c-4479-850f-b3da9c3a3ce5
using LinearAlgebra: dot

# ╔═╡ 26392571-f135-4baa-8a38-a05e55e2dfaa
begin
	using Turing
	using Distributions
...
function psd(x, Δt)
	...
	return power[1:ind], f[1:ind]
end

# ╔═╡ c19a4580-40f8-4f24-ad81-02dcee3d8af3
@bind f Slider(10:10:100)

# ╔═╡ 629286c2-0f35-11ec-27b7-7b1f92592bfb
...
# ╔═╡ 1ef6ac80-db20-4343-98dc-f3f5270fbb06
begin
	power, freq = psd(x_sin, Δt)
	plot(freq, power, title=string("frequency=", f))
end

# ╔═╡ 4ee93fe0-dd75-436f-912a-79038fb265f3
md"""
# Ornstein-Uhlenbeck Process
Compute power spectra of OU process the standard way and with the auto-covariance function.

"""

# ╔═╡ a7dfaf22-6029-445e-951f-cac71a0427d9
function OU_process!(ou, μ, σ, τ, Δt)
	Random.seed!(1)
	for i = 2:n
		ou[i] = ou[i-1] + (1 - exp(-Δt/τ))*(μ - ou[i-1]) + σ * sqrt(1 - exp(-2*Δt/τ)) * randn()
	end
end

# ╔═╡ 8003bf1f-f736-496e-83d5-fbb5079ddedd
begin
	τ_x = 0.01
	τ_y = 5
	x_ou = zeros(n)
	y_ou = zeros(n)
	OU_process!(x_ou, 0, 2, τ_x, Δt)
	OU_process!(y_ou, 0, 2, τ_y, Δt)
	plot(t, [x_ou, y_ou], label=[string("tau=", τ_x) string("tau=", τ_y)])
end

# ╔═╡ fa6f20c8-090c-4761-a0e4-63cec30e793a
begin
	p = psd(x_ou, Δt)
	plot(freq, p)
end

# ╔═╡ a02643e0-f1b2-4abb-9b0c-e2ad31757ca8
# now compute the PSD through Wiener-Khinchin theorem
begin
	r = autocov(x_ou, 1:n-1)
	plot(t[1:n-1], r, title="auto-covariance function")
end

# ╔═╡ 1e7bc07d-b91c-4472-903d-a1252f28afc0
begin
	psd_r = abs.(fft(r))/length(r)
	plot(freq[1:round(Int, length(r)/2)], psd_r[1:round(Int, length(r)/2)])
end

# ╔═╡ a667da6f-2557-4355-98b6-3a02d40465ed
md"""
# Cross-spectral density
Evaluate cross spectral densities
"""

# ╔═╡ afb7ee80-2241-4724-bfb3-ec0d3e6104bc
begin
	const ComplexVector{T<:Complex} = AbstractArray{T,1}
	const RealVector{T<:Real} = AbstractArray{T,1}
end

# ╔═╡ 2accab7b-d9a0-4c5b-bb1d-659417884c41
# cross-spectral density
function csd!(r::ComplexVector, x::RealVector, y::RealVector)
    lx = length(x)
...
# ╔═╡ 5164b7ca-beb2-48b1-9dec-184b55adea70
begin
	z_ou = x_ou + y_ou
	g = zeros(Complex, length(z_ou))
	g = csd!(g, y_ou, z_ou)   # need to implement Welch's method
end

# ╔═╡ 467dceaf-8a0b-41a0-ba1c-23e58c8821cd
plot(freq,real.(g[1:length(freq)]))

# ╔═╡ 41224f7d-24bc-4fbb-8370-779048f65a70
length(freq)

# ╔═╡ ca1d3d81-9549-4b36-8efd-56786c5943e0
begin
	x = cos.(2*pi*t*200)+randn(length(t))
	y = cos.(2*pi*t*100)+randn(length(t))
	g2 = zeros(Complex, length(x))
	csd!(g2, x, y)
end

# ╔═╡ ea3468b2-c562-4d6d-b8f1-eeb86e7be2fd
md"""
